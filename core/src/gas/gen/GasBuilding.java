...
public class GasBuilding extends Building {
    ...
    public boolean acceptGas(Building source, Gas gas) {
        return (source instanceof GasBuilding || source == null) && acceptGas((GasBuilding)source, gas);
    }
    public boolean acceptGas(GasBuilding source, Gas gas) {
    ...
    public float moveGas(Building n, Gas gas) {
        if (n instanceof GasBuilding) {
            ...
            if (next.team == team && next.block.hasGasses && gasses.get(gas) > 0.0F) {
