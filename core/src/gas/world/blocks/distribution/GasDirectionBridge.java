...
public class GasDirectionBridge extends GasBlock{
    ...
    public GasDirectionBridge(String name){
        super(name);
        update = true;
        solid = true;
        rotate = true;
    ...
    public void drawPlace(int x, int y, int rotation, boolean valid, boolean line){
        ...
        for(int i = 1; i <= range; i++){
            ...
            if(other != null && other.build instanceof GasDirectionBridgeBuild build && build.block == this && build.team == player.team()){
                length = i;
                found = other.build;
                break;
            }
    ...
    public class GasDirectionBridgeBuild extends GasBuilding{
        ...
        @Nullable
        public GasDirectionBridgeBuild findLink(){
            for(int i = 1; i <= range; i++){
                Tile other = tile.nearby(Geometry.d4x(rotation) * i, Geometry.d4y(rotation) * i);
                if(other != null && other.build instanceof GasDirectionBridgeBuild build && build.block == GasDirectionBridge.this && build.team == team){
                    return build;
                }
            }
            return null;
